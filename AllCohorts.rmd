...
# How does TI performance relate to SM score? 
...
#posttestResults <- readRDS("posttest39.rds")
#posttestResults1 <- posttestResults[posttestResults$group=="LE" & posttestResults$test=="TI1", ]
#posttestResults2 <- posttestResults[posttestResults$group=="LL" & posttestResults$test=="TI2", ]
#posttestResults3 <- rbind(posttestResults1,posttestResults2)
posttestResults<- posttestResults %>% group_by(user) %>% summarise(TrigID = mean(correct)*100)

pretestData <- readRDS("pretest.rds")

sm <- pretestData %>%
  select("user", "MarkXYII", "MarkThetaII", "Right Triangle Trig III", "Right Triangle Trig V")
sm$sm <- rowMeans(sm[,2:5])

postSM <- merge(posttestResults,sm,by="user")

lmPostSM = lm(TrigID ~ sm, data=postSM)

plot(TrigID ~ sm, data=postSM,main="SM Score v. Trig ID Posttest")
abline(lm(TrigID ~ sm, data=postSM))
summary(lmPostSM)
```
...
posttestData <- readRDS("posttestAll.rds")
pretestData <- readRDS("pretest.rds")
...
allData <- merge(lessonDataAverageAll,sm,by="user")
ch14DataSum <- merge(lessonDataAverageCh14,sm, by="user")
ch56DataSum <- merge(lessonDataAverageCh56,sm, by="user")

##predict lesson performance with sm score

lm1 = lm(PercentCorrect ~ sm, data=allData)
lm2 = lm(PercentCorrect ~ sm, data=ch14DataSum)
lm3 = lm(PercentCorrect ~ sm, data=ch56DataSum)

plot(PercentCorrect ~ sm, data=ch14DataSum,main="SM Score v. Ch 1-4 Performance")
abline(lm(PercentCorrect ~ sm, data=ch14DataSum,main="Ch 1-4"))
summary(lm2)

plot(PercentCorrect ~ sm, data=ch56DataSum, main="SM Score v. Ch 5-6 Performance")
abline(lm(PercentCorrect ~ sm, data=ch56DataSum, main="Ch 5-6"))
summary(lm3)
...
# posttest <- readRDS("posttest39.rds")
# 
# posttestGroupA <- posttest[posttest$group=="LE" & posttest$test=="TI1",]
# posttestGroupA <- posttestGroupA %>% group_by(user) %>% summarise(TrigID = mean(correct)*100)
# 
# posttestGroupB <- posttest[posttest$group=="LL" & posttest$test=="TI2",]
# posttestGroupB <- posttestGroupB %>% group_by(user) %>% summarise(TrigID = mean(correct)*100)
# 
# posttestResults <- rbind(posttestGroupA,posttestGroupB)

posttestResults <- readRDS("posttestAll.rds")
posttestResults<- posttestResults %>% group_by(user) %>% summarise(TrigID = mean(correct)*100)

ch14DataSum <- merge(lessonDataAverageCh14,posttestResults,by="user")
ch56DataSum <- merge(lessonDataAverageCh56,posttestResults,by="user")
allDataSum <- merge(lessonDataAverageAll,posttestResults,by="user")
names(ch14DataSum)[2] <- "PercentCorrectCh14"
names(ch56DataSum)[2] <- "PercentCorrectCh56"

lm5 = lm(TrigID ~ PercentCorrectCh14, data=ch14DataSum)
plot(TrigID ~ PercentCorrectCh14, data=ch14DataSum,main="Ch1-4 Lesson Performance v Posttest")
abline(lm(TrigID ~ PercentCorrectCh14, data=ch14DataSum))
summary(lm5)

lm6 = lm(TrigID ~ PercentCorrectCh56, data=ch56DataSum)
plot(TrigID ~ PercentCorrectCh56, data=ch56DataSum,main="Ch5-6 Lesson Performance v Posttest")
abline(lm(TrigID ~ PercentCorrectCh56, data=ch56DataSum))
summary(lm6)
